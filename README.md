...
v0.9.1 在 AIRP 实际启用且估算收益为非负时，把成本路由得出的上下文节省率写入注入块。Skill 会在最终回答末尾原样追加一行估算值；跳过、证据不足和失败路径不显示。该值基于 UTF-8 字节与固定工具封装的代理模型，不是平台计费 Token 或整段对话费用。

## Codex 插件

项目已包含可安装插件 [plugins/airp](plugins/airp)。当前版本通过 `UserPromptSubmit` Hook 在第一次模型请求前完成本地索引和自适应检索，从根本上消除分析型 MCP 的“模型 → 工具 → 模型”额外往返。默认插件不加载 MCP；独立 CLI 和 MCP 适配器仍保留供显式兼容与事务流程使用。当前源码插件版本为 `0.9.1+codex.20260916141149`；安装后新建 Codex 任务加载。
