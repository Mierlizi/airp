...
When an enabled `<airp-context>` includes `response_footer`, place its value verbatim as the final paragraph of the final user-facing answer. Before sending, ensure that exact value occurs once in the entire answer; move or remove any earlier occurrence. Do not append it to commentary or progress updates. Do not synthesize a saving footer when the field is absent; partial, abstained, skipped, and failed routes intentionally omit it. The percentage estimates context-input savings against AIRP's deterministic native-retrieval baseline, not billed tokens or total conversation cost.
